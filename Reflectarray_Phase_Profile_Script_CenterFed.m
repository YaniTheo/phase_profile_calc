%% Phase profile calculation for rectangular reflectarray with feed positioned at (x,y,z) = (0,0,R0)
clc
clear
...
%Set phase center(feed distance)
R0 = 117.14;
...
for i=-resolution/2:resolution/2-1
    for j=-resolution/2:resolution/2-1
        x(i+resolution/2+1) = (i)*4.7;
        y(j+resolution/2+1) = (j)*4.7;
        R(i+resolution/2+1,j+resolution/2+1) = sqrt((x(i+resolution/2+1)^2+y(j+resolution/2+1)^2)+R0^2);
        phase_profile(i+resolution/2+1,j+resolution/2+1) =rad2deg(k*R(i+resolution/2+1,j+resolution/2+1)) -rad2deg(k*(sin(theta_0)*(x(i+resolution/2+1)*cos(phi_0)+y(j+resolution/2+1)*sin(phi_0))))+phi_con;
